...
$buku = $db->getALL("SELECT buku.*, kategori.kategori FROM buku, kategori WHERE buku.idKategori = kategori.idKategori ORDER BY idBuku ASC LIMIT " . $awaldata . "," . $jumlahdataperhalaman);
		...
		<div class="content">
			...
			<table border="0" cellpadding="10" cellspacing="0">
				...
				<?php foreach($buku as $data) : ?>
					<tr>
						<td><?= $i; ?></td>
						<td><img src="../../assets/img/<?php if (!$data["gambarBuku"]) {echo 'default-book.png';} else {echo $data['gambarBuku'];} ?>" width="75" onclick="viewImage(<?=$i?>)"></td>
						<td width=""><?= $data['judulBuku']; ?></td>
						<td><?= $data['kategori']; ?></td>
						...
						<?php if($_SESSION['level'] == 'Admin') { ?>
							<td>
								<ul class="pagination pagination-sm">
									...
									<li class="page-item"><a class="page-link book-stock"><?php $stokBuku = $db->rowCOUNT("SELECT * FROM detailbuku WHERE status='Ada' AND idBuku = " . $data['idBuku'] . ""); echo $stokBuku; ?></a></li>
